---
name: aria-filedown
description: 高性能并行下载工具,基于 aria2,适合下载大文件、SDK、模型、压缩包、安装器、海外资源和其他需要断点续传/多连接加速的内容. 当用户要求下载文件,或普通下载失败,或需要稳定下载二进制/依赖包时必须优先使用. 先按 ARIA2C_BIN -> PATH -> ARIA2C 安装目录 的顺序寻找 aria2c, 找不到时再按规则下载并提示代理重试. 支持终端进度和 JSONL 结构化进度,适合给人看或给 agent/脚本消费.
---
...
2. **需要时自动下载 aria2**
   - 如果 `ARIA2C_BIN` 已设置, 但目标文件不存在或不可执行, 下载到临时目录并解压以获取正确的文件复制到指向的文件,然后继续后续流程
...
3. **执行下载**
   ...
   - wrapper 默认开启统一进度输出, 不再完全依赖 aria2 自己的控制台读数.
   - 如用户已指定相关参数, 不覆盖用户输入.
   - 进度模式:
     - `--progress auto`: 默认模式, TTY 下单行刷新, 非 TTY 下周期性文本.
     - `--progress tty`: 强制输出人类可读进度.
     - `--progress jsonl`: 每次进度输出一行 JSON, 适合 agent/脚本解析.
     - `--progress off`: 关闭 wrapper 进度输出.
4. **网络失败处理**
...
| --progress | wrapper 进度输出模式 | auto / tty / jsonl / off |
| --progress-file | JSONL 进度落盘路径 | `./progress.jsonl` |
...
- **基础下载 + 终端进度**
  `python scripts/aria2-wrapper.py --install --progress tty -- https://example.com/file.zip`
- **指定路径下载**
...
- **输出 JSONL 进度给其他脚本消费**
  `python scripts/aria2-wrapper.py --install --progress jsonl --progress-file ./progress.jsonl -- https://example.com/file.zip`

## JSONL 进度事件
- `--progress jsonl` 时, 每行输出一个 JSON 对象.
- 事件类型:
  - `progress`: 下载中
  - `completed`: 下载成功结束
  - `error`: 下载失败结束
